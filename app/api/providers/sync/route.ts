...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await safeProviderFetch(auth.baseURL + '/models', { headers: auth.apiKey ? { authorization: 'Bearer ' + auth.apiKey } : {}, signal: AbortSignal.timeout(10_000) }, def.local);
